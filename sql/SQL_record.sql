...
CREATE TABLE `text_vector_log`
(
    `id`          bigint NOT NULL COMMENT '主键',
    `text`        text COMMENT '文本',
    `vector`      text COMMENT '向量',
    `create_time` datetime COMMENT '创建时间',
    `source`      varchar(255) DEFAULT NULL COMMENT '来源',
    `remark`      varchar(512) DEFAULT NULL COMMENT '备注',
    PRIMARY KEY (`id`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4 COMMENT ='文本向量日志';


create table car_sales
(
    sale_id          bigint auto_increment comment '销售记录ID'
        primary key,
    company_id       int            not null comment '厂商ID（关联 car_company.company_id）',
    car_id           int            null comment '车型ID（关联 electric_car.id，为空表示厂商整体统计）',
    sale_year        smallint       not null comment '销售年份',
    sale_month       tinyint        not null comment '销售月份（1-12）',
    region           varchar(50)    not null default 'Global' comment '销售地区（如 China / Europe / North America / Global）',
    sales_volume     int            not null default 0 comment '销量（辆）',
    sales_revenue    decimal(15, 2) null comment '销售额（万元人民币）',
    avg_price        decimal(10, 2) null comment '成交均价（万元人民币）',
    market_share_pct decimal(5, 2)  null comment '市场占有率（%）',
    yoy_volume       int            null comment '同比销量增加（辆，当月销量 - 去年同月销量）',
    yoy_growth_pct   decimal(6, 2)  null comment '同比增长率（%，正为增长，负为下滑）',
    mom_volume       int            null comment '环比销量增加（辆，当月销量 - 上月销量）',
    mom_growth_pct   decimal(6, 2)  null comment '环比增长率（%，正为增长，负为下滑）',
    energy_type      varchar(20)    null comment '能源类型（BEV / PHEV / EREV / ICE）',
    data_source      varchar(100)   null comment '数据来源（如乘联会、厂商公告等）',
    ...
    unique key uk_sale (company_id, car_id, sale_year, sale_month, region),
    index idx_year_month (sale_year, sale_month),
    index idx_company (company_id),
    index idx_car (car_id),
    index idx_region (region)
) comment '汽车销售记录表' charset = utf8mb4;
show tables;
